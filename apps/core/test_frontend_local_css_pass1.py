...
@pytest.mark.django_db
def test_public_pages_load_local_css_before_cdn():
    for path in PUBLIC_PATHS:
        body = Client().get(path).content.decode("utf-8")
        idx_local = body.find("/static/css/site.css")
        idx_cdn = body.find("cdn.tailwindcss.com")
        assert idx_local != -1, f"{path} missing local site.css"
        assert idx_cdn != -1, f"{path} missing CDN reference (regression risk)"
        # Local must come BEFORE CDN so the local rules are the
        # baseline and the CDN only adds extra utilities.
        assert idx_local < idx_cdn, f"{path} loads CDN before local CSS"
