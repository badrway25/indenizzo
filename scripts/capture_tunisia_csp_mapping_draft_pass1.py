"""Capture screenshots for F-tunisia-csp-livre-ix-mapping-draft-pass1.

Verifies the public surface after the Tunisia inheritance mapping
DRAFT was committed (CSP Livre IX, articles 122–143, all rules
blocked, EU 650 declared as non-load-bearing). The TN public funnel
must still produce ``unavailable_requires_legal_validation`` and the
result page must not surface the mapping internals or the new
diagnostic. Tailwind CDN is route-aborted to enforce the
...
BASE_URL = "http://127.0.0.1:48112"
OUTDIR = REPO_ROOT / "docs" / "screenshots" / "live_qa" / "tunisia_csp_mapping_draft_pass1"
