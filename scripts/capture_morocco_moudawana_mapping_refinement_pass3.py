"""Capture screenshots for F-morocco-moudawana-mapping-refinement-pass3.

Confirm the public surface still presents the right MA messaging
after the mapping draft has been refined: split article 346 +
explicit unsupported_mechanisms + per-rule activation_blockers. The
calculator stays ``unavailable_requires_legal_validation`` and no
automatic shares surface. Tailwind CDN is route-aborted to enforce
the local-CSS-only baseline.
"""
...
OUTDIR = (
    REPO_ROOT / "docs" / "screenshots" / "live_qa" / "morocco_moudawana_mapping_refinement_pass3"
)
...
DESKTOP = [
    ...
    ("02_wizard_ma_inheritance", "", "/wizard/ma/inheritance/", None),
    ("03_result_ma_unavailable", "", "/wizard/ma/inheritance/", MA_PAYLOAD),
    ("04_wizard_ma_inheritance_ar", "/ar", "/wizard/ma/inheritance/", None),
    ("05_result_ma_unavailable_ar", "/ar", "/wizard/ma/inheritance/", MA_PAYLOAD),
]
